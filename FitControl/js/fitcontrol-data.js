...
function nomeAlunoPorId(idAluno) {
  ...
  return alunos.find(aluno => String(aluno.idAluno) === String(idAluno))?.nome || "Aluno não encontrado";
}

function nomeProfessorPorId(idProfessor) {
  const professores = JSON.parse(localStorage.getItem("professores")) || [];
  return professores.find(prof => String(prof.idProfessor) === String(idProfessor))?.nome || "Professor não encontrado";
}
